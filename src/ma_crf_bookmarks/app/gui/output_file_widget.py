...
class OutputFileWidget(QGroupBox):
    def __init__(
        self,
    ...
    def valid_output(self) -> bool:
        if self.text_box.text():
            ...
            crf_dir = os.path.dirname(path)
            suffix = path.split(".")[-1]
            if os.path.exists(crf_dir) and suffix == "pdf":
                return True
        return False
    ...
    def select_output(self):
        ...
        if self.valid_output() and self.parent_widget.crf_path and self.parent_widget.sds_path:
